...
#define sama5d2_pmc_write(off, val) writel(val, IOMEM(SAMA5D2_BASE_PMC) + off)
#define sama5d2_pmc_read(off) readl(IOMEM(SAMA5D2_BASE_PMC) + off)
...
void sama5d2_ddr2_init(struct at91_ddramc_register *ddramc_reg_config)
{
	...
	at91_ddram_initialize(IOMEM(SAMA5D2_BASE_MPDDRC), IOMEM(SAMA5_DDRCS),
			      ddramc_reg_config);
}

void sama5d2_pmc_init(unsigned int msc)
{
	at91_pmc_init(IOMEM(SAMA5D2_BASE_PMC), AT91_PMC_LL_SAMA5D2);
