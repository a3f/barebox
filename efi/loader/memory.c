...
#define pr_fmt(fmt) "efi-memory: " fmt
				...
				const char *name)
{
	...
	scnprintf(namebuf, sizeof(namebuf), "efi%lu-%s%c%s", efi_memory_map_key,
...
static int free_efi_only(struct resource *res, void *data)
{
	...
	if (!(res->flags & IORESOURCE_EFI_ALLOC)) {
		pr_err("refusing to free non-EFI allocated resource %s at 0x%llx\n",
			res->name, res->start);
		*nfreed = -1;
		return false;
	}
