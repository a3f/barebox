...
static efi_status_t EFIAPI efi_stall(unsigned long microseconds)
{
	...
	// TODO move efi_timer_check into poller. yup
			       ...
			       u16 **exit_data, int flags)
{
	...
	if (flags & EFI_VERBOSE_RUN)
		printf("Jumping into 0x%p *%p\n", entry, info);
	else
		EFI_PRINT("Jumping into 0x%p *%p\n", entry, info);


	remap_range(info->image_base, info->image_size, MAP_ARCH(2)); // FIXME: RWX
