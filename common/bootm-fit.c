...
static enum filetype bootm_fit_update_os_header(struct image_data *data)
{
	...
	free(data->os_header);
	data->os_header = xmemdup(header, PAGE_SIZE);
					     ...
					     unsigned flags)
{
	...
	if (unlikely(zero_page_contains((ulong)load_addr)))
		zero_page_memcpy(load_addr, data + offset, size - offset);
	else
		memcpy(load_addr, data + offset, size - offset);

	return size - offset; /* Return actual bytes written */
}
