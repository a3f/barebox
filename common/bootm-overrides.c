 ...
 * @override: override path (may include @config suffix)
 *
 * Applies a bootm.image override according to these rules:
 * - If the override is not a FIT image, it replaces only data->os
 * - If both original and override are FIT images with the same top-level
 *   description, the override replaces os_fit and reparsing occurs (os_part
 *   is inherited from original if override doesn't specify one)
 * - If both are FIT images with different descriptions, components are
 *   extracted from the override using the appropriate configuration
 *
			  ...
			  const struct bootm_overrides *overrides)
{
	...
	report_active_overrides(data, overrides);
