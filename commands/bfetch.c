...
static void print_features(unsigned *line)
{
	...
	ret = glob("/dev/ubootvar", 0, NULL, &g);
	if (!ret) {
		if (g.gl_pathc)
...
static void print_tee(unsigned *line)
{
	struct device *dev, *teedev = NULL;
	const char *name;
	struct bobject *bret;
	int ret;
	...
	ret = structio_devinfo(&bret, teedev);
	if (ret)
		return;

	name = bobject_get_param(bret, "impl.name");
	if (name)
