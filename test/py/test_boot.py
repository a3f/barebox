...
def test_bootm_initrd_dt_properties(barebox, bootm_mock, boot_testdata):
    ...
    # Run bootm
    _, _, ret = barebox.run(f"bootm -f -r {initrd_path} {image_path}")
    assert ret == 0, "bootm failed"
...
def test_boot_script_with_bootm(barebox, bootm_mock, boot_testdata):  # noqa: ARG001
    ...
    # The script's bootm -f call will succeed with test handler, but then
    ...
    barebox.run_check("echo -a /env/boot/testscript 'bootm -f'")
...
def test_boot_script_without_bootm(barebox, bootm_mock, boot_testdata):  # noqa: ARG001
    ...
    # The script must call bootm -f since boot command doesn't have -f
    barebox.run_check("echo -a /env/boot/testscript2 'bootm -f'")
