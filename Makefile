...
$(filter-out __all __sub-make $(lastword $(MAKEFILE_LIST)), $(MAKECMDGOALS)) __all: __sub-make
	@:
...
ifneq ($(filter all __all modules %compile_commands.json,$(MAKECMDGOALS)),)
  KBUILD_MODULES := 1
endif
...
all: $(BAREBOX_PROPER) images $(symlink-y)
else
