...
:ref:`state_framework`. For that reason, an empty device tree is compiled into
the EFI payload, which can be populated at build time with the
``CONFIG_EXTERNAL_DTS_FRAGMENTS`` option, e.g.::
...
``efi_payload_dts`` macro, which is defined while barebox' own EFI device tree
is compiled:

.. code-block:: text

  #ifdef efi_payload_dts
  / {
          aliases {
                  state = &state;
          };
...
Should the built-in device tree describe no *state*, barebox falls back to
reading a devicetree file ``state.dtb`` out of the ``EFI/barebox/`` directory
on the :term:`ESP`.

barebox' internal device tree is exported in flattened form in the
``barebox-dtb`` EFI variable under the barebox vendor GUID, so the operating
system can be configured by the same description. Under Linux, it's readable at
