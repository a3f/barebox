...
#define sconfig_debug(policy, opt, fmt, ...) \
	pr_debug("(%s) %s: " fmt, \
		 (policy) ? (policy)->name : "default", \
		 sconfig_name(opt), ##__VA_ARGS__)

#ifdef CONFIG_SECURITY_POLICY_DEFAULT_PERMISSIVE
#define sconfig_err			pr_err
#define sconfig_warn_once		pr_warn_once
#define sconfig_WARN			WARN
#else
#define sconfig_err(fmt, ...)		panic(pr_fmt(fmt), ##__VA_ARGS__)
#define sconfig_warn_once(fmt, ...)	panic(pr_fmt(fmt), ##__VA_ARGS__)
#define sconfig_WARN			BUG
#endif

struct sconfig_list_entry {
...
bool is_allowed(const struct security_policy *policy, unsigned option)
{
	...
	if (policy) {
		...
		sconfig_debug(policy, option, "%s for %pS\n",
...
{
	const struct sconfig_list_entry *entry;

	list_for_each_entry(entry, &sconfig_list, list) {
...
int security_policy_select(const char *name)
{
	...
	if (!policy) {
		sconfig_err("Policy '%s' not found!\n", name);
		return -ENOENT;
	}
...
int __security_policy_register(const struct security_policy policy[])
{
	...
	do {
		struct sconfig_list_entry *entry;

		if (security_policy_get(policy->name)) {
			sconfig_err("policy '%s' already registered\n", policy->name);
			ret = -EBUSY;
			continue;
		}
		...
		entry->policy = policy;
		list_add_tail(&entry->list, &sconfig_list);
				    ...
				    unsigned long opt, void *data)
{
	...
	sconfig_debug(active_policy, opt, "calling %pS to %s\n",
