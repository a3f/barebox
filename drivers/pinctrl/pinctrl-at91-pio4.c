				       ...
				       struct device_node *np)
{
	...
	// TODO: why check for pinconf_to_config_argument
	...
	list = of_get_property(np, "pinmux", &npins);
	if (!list)
		return -EINVAL;
...
static int pinctrl_at91_pio4_probe(struct device_d *dev)
{
	...
	dev_info(dev, "AT91 PIO4 pinctrl driver registered\n"); // FIXME remove
	...
	dev_info(dev, "AT91 PIO4 gpio driver registered\n"); // FIXME remove
