...
// SPDX-Comment: Origin-URL: https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/drivers/platform/chrome/cros_ec_spi.c?id=e19ceeb1c0f63e3e15b197c5f34797134b51ba0e
 ...
 */
#define EC_MSG_DEADLINE_NS		(200 * MSECOND)
...
/*
  ...
  * safe.
  */
#define EC_SPI_RECOVERY_TIME_NS		(200 * USECOND)
				      ...
				      int need_len)
{
	...
	while (true) {
		...
		if (is_timeout(start, EC_MSG_DEADLINE_NS)) {
...
#define cros_ec_xfer_high_pri(ec_dev, ec_msg, fn)	(fn)(ec_dev, ec_msg)
...
static int cros_ec_spi_probe(struct device *dev)
{
	...
	ec_dev->slice = spi_device_slice(spi);
...
static struct driver cros_ec_driver_spi = {
	.name	= "cros-ec-spi",
	.of_match_table = cros_ec_spi_of_match,
	.probe		= cros_ec_spi_probe,
};

device_spi_driver(cros_ec_driver_spi);
