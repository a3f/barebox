...
static int mtk_disp_pwm_probe(struct device *dev)
{
	...
	mdp->base = dev_platform_ioremap_resource(dev, 0);
	if (IS_ERR(mdp->base))
		return PTR_ERR(mdp->base);

	mdp->clk_main = clk_get(dev, "main");
	if (IS_ERR(mdp->clk_main))
		return dev_err_probe(dev, PTR_ERR(mdp->clk_main),
				     "Failed to get main clock\n");

	mdp->clk_mm = clk_get(dev, "mm");
	if (IS_ERR(mdp->clk_mm))
		return dev_err_probe(dev, PTR_ERR(mdp->clk_mm),
				     "Failed to get mm clock\n");

	ret = clk_prepare_enable(mdp->clk_main);
	if (ret)
		return dev_err_probe(dev, ret, "Can't prepare enable main clk\n");

	ret = clk_prepare_enable(mdp->clk_mm);
	if (ret)
		return dev_err_probe(dev, ret, "Can't prepare enable mm clk\n");

	dev->priv = mdp;

	mdp->data = of_device_get_match_data(dev);
	mdp->chip.dev = dev;
	mdp->chip.ops = &mtk_disp_pwm_ops;
	mdp->chip.devname = of_alias_get(dev->of_node);
	if (!mdp->chip.devname)
		mdp->chip.devname = xasprintf("disp_pwm%d", dev->id > 0 ? dev->id : 0);
...
static struct driver mtk_disp_pwm_driver = {
	.name = "mediatek-disp-pwm",
	.of_match_table = mtk_disp_pwm_of_match,
	.probe = mtk_disp_pwm_probe,
};

device_platform_driver(mtk_disp_pwm_driver);
