...
static struct bus_type faux_bus_type = {
	.name		= "faux",
	.match		= faux_match,
	.probe		= faux_probe,
	.remove		= faux_remove,
...
/**
 * faux_device_create - Create and register with the driver
				       ...
				       const struct faux_device_ops *faux_ops)
{
	...
	faux_obj = calloc(sizeof(*faux_obj), 1);
	if (!faux_obj)
		return NULL;
...
static int __init faux_bus_init(void)
{
	int ret;
	...
	return driver_register(&faux_driver);
}
pure_initcall(faux_bus_init);
