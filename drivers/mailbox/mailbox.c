...
int mbox_send_message(struct mbox_chan *chan, void *mssg)
{
	...
	if (chan->cl->tx_block) {
		...
		if (!chan->cl->tx_tout)
			wait_ms = NSEC_PER_SEC;
		else
			wait_ms = chan->cl->tx_tout;

		while (!chan->tx_complete) {
			if (is_timeout(start, wait_ms * NSEC_PER_MSEC))
				t = -ETIME;
			if (ctrlc())
				t = -EINTR;
...
struct mbox_chan *mbox_request_channel(struct mbox_client *cl, int index)
{
	...
	if (of_parse_phandle_with_args(dev->device_node, "mboxes",
	...
	list_for_each_entry(mbox, &mbox_cons, node)
		if (mbox->dev->device_node == spec.np) {
...
struct mbox_chan *mbox_request_channel_byname(struct mbox_client *cl,
					      const char *name)
{
	struct device_node *np = cl->dev->device_node;
