...
static int clk_mt8192_apmixed_probe(struct device *pdev)
{
	...
	r = mtk_clk_register_gates(pdev, node, apmixed_clks,
...
static void clk_mt8192_apmixed_remove(struct device *pdev)
{
	struct device_node *node = pdev->of_node;
	struct clk_hw_onecell_data *clk_data = dev_get_drvdata(pdev);
...
static struct driver clk_mt8192_apmixed_drv = {
...
postcore_platform_driver(clk_mt8192_apmixed_drv);
