...
static struct file_list_entry *ums_file_entry;
...
static int fsg_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	common = fsg_common_init(fsg->common, c->cdev);
	if (IS_ERR(common))
	...
	file_list_for_each_entry(ums_files, fentry) {
		...
		i = stat(fentry->filename, &st);
		if (i < 0) {
			pr_warn("Error stating file = %pe\n", ERR_PTR(i));
			continue;
		}

		if (!S_ISBLK(st.st_mode)) {
			pr_warn("Not a block device\n");
			continue;
		}

		if (ums_file_entry->flags & FILE_LIST_FLAG_CREATE)
			flags |= O_CREAT | O_TRUNC;

		i = open(fentry->filename, flags);
		if (i < 0) {
			pr_warn("Error stating file = %pe\n", ERR_PTR(i));
			continue;
		}
		...
		pr_info("LUN %d, count %#x\n",
		       ums_count, ums[ums_count].num_sectors);
