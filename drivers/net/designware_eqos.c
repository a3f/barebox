...
#define EQOS_MAC_MDIO_ADDR(reg)		((addr << 21) & GENMASK(25, 21))
#define EQOS_MAC_MDIO_REG(reg)		((reg << 16) & GENMASK(20, 16))
#define EQOS_MAC_MDIO_CLK_CSR(clk_csr)	((clk_csr << 8) & GENMASK(11, 8))
...
static int eqos_mdio_read(struct mii_bus *bus, int addr, int reg)
{
	...
	if (ret) {
		dev_err(&eqos->netdev.dev, "MDIO not idle at entry\n");
		return ret;
	}
	...
	miiaddr |= EQOS_MAC_MDIO_CLK_CSR(eqos->ops->clk_csr);
	miiaddr |= EQOS_MAC_MDIO_ADDR(addr) | EQOS_MAC_MDIO_REG(reg);
	miiaddr |= MII_BUSY;
	...
	ret = eqos_mdio_wait_idle(eqos);
	if (ret) {
		dev_err(&eqos->netdev.dev, "MDIO read didn't complete\n");
		return ret;
	}
...
static void eqos_set_full_duplex(struct eqos *eqos)
{
	setbits_le32(&eqos->mac_regs->config, EQOS_MAC_CONFIGURATION_DM);
}

static void eqos_set_half_duplex(struct eqos *eqos)
{
...
static void eqos_set_gmii_speed(struct eqos *eqos)
{
	clrbits_le32(&eqos->mac_regs->config,
		     EQOS_MAC_CONFIGURATION_PS | EQOS_MAC_CONFIGURATION_FES);
}

static void eqos_set_mii_speed_100(struct eqos *eqos)
{
	setbits_le32(&eqos->mac_regs->config,
		     EQOS_MAC_CONFIGURATION_PS | EQOS_MAC_CONFIGURATION_FES);
}

static void eqos_set_mii_speed_10(struct eqos *eqos)
{
...
void eqos_adjust_link(struct eth_device *edev)
{
	...
	case SPEED_10:
		...
		break;
	default:
		dev_err(&edev->dev, "invalid speed %d\n", speed);
		return;
	}
}
...
static int eqos_dma_reset(struct eqos *eqos)
{
	void __iomem *dma_bus_mode = eqos->regs + DMA_BUS_MODE;

	u32 value = readl(dma_bus_mode);
	int limit;

	/* DMA SW reset */
	value |= DMA_BUS_MODE_SFT_RESET;
	writel(value, dma_bus_mode);
	limit = 10;
	while (limit--) {
		if (!(readl(dma_bus_mode) & DMA_BUS_MODE_SFT_RESET))
			break;
		mdelay(10);
	}
...
int eqos_start(struct eth_device *edev)
{
	...
	ret = readl_poll_timeout(&eqos->dma_regs->mode, mode_set,
				 ...
				 10 * USEC_PER_MSEC);
	if (ret) {
		dev_err(&edev->dev, "EQOS_DMA_MODE_SWR stuck: 0x%08x\n", mode_set);
		return ret;
	}
...
int eqos_init(struct eqos *eqos)
{
	int ret;

	ret = eqos_init_resources(eqos);
	if (ret) {
		dev_err(&eqos->netdev.dev, "eqos_init_resources() failed: %s\n",
			strerror(-ret));
		return ret;
	}

	if (eqos->ops->init)
		ret = eqos->ops->init(eqos);

	return ret;
}

static int eqos_probe_dt(struct device_d *dev, struct eqos *eqos)
{
...
struct eqos *eqos_drv_probe(struct device_d *dev, const struct eqos_ops *ops, void *priv)
{
...
void eqos_drv_remove(struct device_d *dev)
{
