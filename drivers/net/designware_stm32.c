...
static unsigned long eqos_get_tick_clk_rate_stm32(struct eqos *eqos)
{
	return clk_get_rate(to_stm32(eqos)->clks[CLK_STMMACETH].clk);
}

static void eqos_set_mode_stm32(struct eqos *eqos)
{
	...
	case PHY_INTERFACE_MODE_MII:
		...
		dev_dbg(dev, "using PHY_INTERFACE_MODE_MII\n");
		break;
	case PHY_INTERFACE_MODE_GMII:
		val = SYSCFG_PMCR_ETH_SEL_GMII;
		if (priv->eth_clk_sel_reg)
			val |= SYSCFG_PMCR_ETH_CLK_SEL;
		dev_dbg(dev, "using PHY_INTERFACE_MODE_GMII\n");
		break;
	case PHY_INTERFACE_MODE_RMII:
		val = SYSCFG_PMCR_ETH_SEL_RMII;
		if (priv->eth_ref_clk_sel_reg)
			val |= SYSCFG_PMCR_ETH_REF_CLK_SEL;
		dev_dbg(dev, "using PHY_INTERFACE_MODE_RMII\n");
		break;
	...
	case PHY_INTERFACE_MODE_RGMII_TXID:
		...
		if (priv->eth_clk_sel_reg)
			val |= SYSCFG_PMCR_ETH_CLK_SEL;
		dev_dbg(dev, "using PHY_INTERFACE_MODE_RGMII\n");
		break;
	default:
...
static int eqos_init_stm32(struct eqos *eqos)
{
	struct device_d *dev = eqos->netdev.parent;
...
{
	struct eqos *eqos = eqos_drv_probe(dev, &stm32_ops,
					   xzalloc(sizeof(struct eqos_stm32)));
