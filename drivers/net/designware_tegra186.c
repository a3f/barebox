...
static int eqos_calibrate_pads_tegra186(struct eqos *eqos)
{
	...
	ret = readl_poll_timeout(&priv->tegra186_regs->auto_cal_status, active,
				 ...
				 10000);
	if (ret) {
		dev_err(&eqos->netdev.dev, "calibrate didn't start\n");
		goto failed;
	}
	...
	ret = readl_poll_timeout(&priv->tegra186_regs->auto_cal_status, active,
				 ...
				 10000);
	if (ret) {
		dev_err(&eqos->netdev.dev, "calibrate didn't finish\n");
		goto failed;
	}
...
static int eqos_calibrate_link_tegra186(struct eqos *eqos, unsigned speed)
{
	...
	if (ret < 0) {
		dev_err(&eqos->netdev.dev, "clk_set_rate(tx_clk, %lu) failed: %d\n", rate, ret);
		return ret;
	}
...
static unsigned long eqos_get_tick_clk_rate_tegra186(struct eqos *eqos)
{
...
static int eqos_init_tegra186(struct eqos *eqos)
{
...
static int eqos_start_tegra186(struct eth_device *edev)
{
	...
	if (ret < 0) {
		dev_err(&eqos->netdev.dev, "clk_bulk_enable() failed: %s\n", strerror(-ret));
		return ret;
	}

	ret = eqos_clks_set_rate_tegra186(priv);
	if (ret < 0) {
		dev_err(&eqos->netdev.dev, "clks_set_rate() failed: %s\n", strerror(-ret));
		goto err;
	}

	eqos_reset_tegra186(priv, false);
	if (ret < 0) {
		dev_err(&eqos->netdev.dev, "reset(0) failed: %s\n", strerror(-ret));
		goto err_stop_clks;
	}
...
static void eqos_adjust_link_tegra186(struct eth_device *edev)
{
	...
	if (ret < 0) {
		dev_err(&eqos->netdev.dev, "eqos_calibrate_link_tegra186() failed: %d\n", ret);
		return;
	}
}
...
{
	struct eqos *eqos = eqos_drv_probe(dev, &tegra186_ops,
					   xzalloc(sizeof(struct eqos_tegra186)));

	return PTR_ERR_OR_ZERO(eqos);
}
