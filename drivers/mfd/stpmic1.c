...
#define to_stpmic1(a)		container_of(a, struct stpmic1, cdev)
...
static int __init stpmic1_probe(struct device_d *dev)
{
	...
	stpmic1->map = regmap_init(dev, &regmap_stpmic1_i2c_bus,
				     stpmic1, &stpmic1_regmap_i2c_config);

	ret = regmap_register_cdev(stpmic1->map, NULL);
	if (ret)
		return ret;

	ret = regmap_read(stpmic1->map, VERSION_SR, &reg);
	if (ret) {
