...
// SPDX-Comment: Origin-URL: https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/drivers/input/matrix-keymap.c?id=40a6bb10d3d124c9adfd9f5e0bb8e020e0ccb712
...
static int matrix_keypad_parse_keymap(const char *propname,
				      ...
				      unsigned short *keymap)
{
	struct device *dev = input_dev->parent;
	struct device_node *np = dev_of_node(dev);
	...
	size = of_property_count_elems_of_size(np, propname, sizeof(u32));
	if (size <= 0) {
	...
	keys = kmalloc_array(size, sizeof(*keys), GFP_KERNEL);
	if (!keys)
		return -ENOMEM;

	error = of_property_read_u32_array(np, propname, keys, size);
	if (error) {
			       ...
			       struct input_dev *input_dev)
{
	...
	if (WARN_ON(!input_dev->parent))
		return -EINVAL;

	if (!keymap) {
		keymap = devm_kcalloc(input_dev->parent,
