...
/*
 * USB HID keyboard driver for barebox
 *
 * (C) Copyright 2001 Denis Peter, MPL AG Switzerland
 * (C) Copyright 2015 Peter Mamonov
 * (C) Copyright 2026 Lucie L. Hartmann, MNT Research GmbH
 */
...
static int usb_kbd_cnt_poll(struct usb_kbd_pdata *data)
{
	...
	return usb_get_report(data->usbdev,
                        iface->desc.bInterfaceNumber,
                        1, // type, input?
                        1, // id, FIXME hardcoded
                        data->new, USB_KBD_BOOT_REPORT_SIZE);
}
			 ...
			 const struct usb_device_id *id)
{
	...
	ret = usb_set_idle(usbdev, iface->desc.bInterfaceNumber, 1, 1); // hardcoded report id 1
	if (ret < 0)
		return ret;
