...
int scmi_driver_register(struct scmi_driver *driver)
{
	...
	retval = platform_driver_register(&driver->driver);
	if (!retval)
...
struct scmi_device *
scmi_device_create(struct device_node *np, struct device_d *parent, int protocol,
		   const char *name)
{
	int id, retval;
	...
	retval = register_device(&scmi_dev->dev);
	if (retval)
		goto put_dev;
