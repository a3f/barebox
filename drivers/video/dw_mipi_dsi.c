...
static void dw_mipi_dsi_init_pll(struct dw_mipi_dsi *dsi)
{
	...
	if (phy_ops->get_esc_clk_rate)
		phy_ops->get_esc_clk_rate(dsi->plat_data->priv_data, &esc_rate);
	else
					   ...
					   u32 hcomponent)
{
	...
	/* TODO: MIPI_DSI_MODE_VIDEO_BURST handling and minimum_lbccs */
...
{
	const struct dw_mipi_dsi_phy_ops *phy_ops = dsi->plat_data->phy_ops;
	struct dw_mipi_dsi_dphy_timing timing = {0x40, 0x40, 0x40, 0x40};
	u32 hw_version;

	if (phy_ops->get_timing)
		phy_ops->get_timing(dsi->plat_data->priv_data, dsi->lane_mbps, &timing);
				 ...
				 struct drm_display_mode *timings)
{
	...
	ret = phy_ops->get_lane_mbps(priv_data, timings, dsi->mode_flags,
