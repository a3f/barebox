...
static u32 convert_drive_impedance_ohm(struct device *dev, u32 dr_ohm)
{
	switch (dr_ohm) {
	case 100:
...
static int rockchip_emmc_phy_probe(struct device *dev)
{
	...
	rk_phy = kzalloc(sizeof(*rk_phy), GFP_KERNEL);
	if (!rk_phy)
		return -ENOMEM;
	...
	if (!of_property_read_u32(dev->of_node, "drive-impedance-ohm", &val))
		rk_phy->drive_impedance = convert_drive_impedance_ohm(dev, val);
	...
	generic_phy = phy_create(dev, dev->of_node, &ops);
...
static struct driver rockchip_emmc_driver = {
	.name	= "rockchip-emmc-phy",
	.probe	= rockchip_emmc_phy_probe,
	.of_compatible = rockchip_emmc_phy_dt_ids,
};
device_platform_driver(rockchip_emmc_driver);
