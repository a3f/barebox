...
static DEFINE_DEV_CLASS(tee_class, "tee");
...
int tee_device_register(struct tee_device *teedev)
{
	...
	class_add_device(&tee_class, &teedev->dev);
...
void tee_device_unregister(struct tee_device *teedev)
{
	...
	class_remove_device(&tee_class, &teedev->dev);
	if (IS_ENABLED(CONFIG_OPTEE_DEVFS))
			...
			const void *data, struct tee_ioctl_version_data *vers)
{
	...
	class_for_each_container_of_device(&tee_class, teedev, dev) {
