...
#define fuzz_test(_name, _func)					\
	static const struct fuzz_test _func##entry		\
...
#else
#define fuzz_test(_name, _func)					\
	static __always_unused void * _unused##entry = _func
#endif
...
static inline void fuzz_test_once(const struct fuzz_test *test, const u8 *data, size_t len)
{
	test->func(data, len);
}
