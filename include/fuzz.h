...
#define fuzz_test_mtdram(_name, _size, _erasesize, _writesize, _func,	\
			 _init)						\
	static struct mtdram *_func##_mtdram_dev;			\
	static __always_unused void _func##_mtdram_init(void)		\
	{								\
		if (!_func##_mtdram_dev)				\
			_func##_mtdram_dev = mtdram_init(_size,		\
					_erasesize, _writesize);	\
		fuzz_call_init(_init);					\
	}								\
	static int _func##_mtdram(const u8 *data, size_t size)		\
	{								\
		struct mtdram *mtdram = _func##_mtdram_dev;		\
		if (!mtdram)						\
			return -ENODEV;					\
		mtdram_setup(mtdram, data, size);			\
		return _func(mtdram_get_mtd(mtdram));			\
	}								\
	__fuzz_test(_name, _func##_mtdram, _func##_mtdram_init)
