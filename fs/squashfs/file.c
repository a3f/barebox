...
static int squashfs_fill_page(struct page *page, struct squashfs_cache_entry *buffer,
			      int offset, int avail)
{
	struct squashfs_page *sq_page = squashfs_page(page);
	int copied;

	copied = squashfs_copy_data(sq_page->buf[offset], buffer, offset, avail);
	memset(sq_page->buf[offset] + avail, 0, PAGE_CACHE_SIZE - avail);
	sq_page->idx++;
	...
	int bytes, int offset)
{
	...
	for (i = 0; i < 32 && bytes > 0; i++,
		...
		ret = squashfs_fill_page(page, buffer, i, avail);
		if (ret)
			return ret;
	}
