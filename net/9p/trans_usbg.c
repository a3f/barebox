			    ...
			    gfp_t gfp_flags)
{
	...
	ret = usb_ep_queue(usb9pfs->in_ep, req);
	if (ret)
		req->context = NULL;
			    ...
			    gfp_t gfp_flags)
{
	...
	ret = usb_ep_queue(usb9pfs->out_ep, req);
			  ...
			  struct f_usb9pfs *usb9pfs)
{
	...
	usb9pfs->in_req = usb_ep_alloc_request(usb9pfs->in_ep);
	if (!usb9pfs->in_req) {
		ret = -ENOENT;
		goto fail;
	}
...
static int p9_wait_for_completion(struct completion *completion, u64 timeout_ns)
{
	int ret;

	ret = wait_on_timeout(timeout_ns, completion_done(completion) || ctrlc());
	if (ret)
		return ret;
...
static int p9_usbg_request(struct p9_client *client, struct p9_req_t *p9_req)
{
	...
	ret = p9_wait_for_completion(&usb9pfs->received, 5 * SECOND);
	if (ret)
		return ret;
	...
	ret = p9_wait_for_completion(&usb9pfs->send, 5 * SECOND);
	if (ret)
		return ret;
...
static int p9_usbg_poll(struct p9_client *client)
{
	/*
	 * USB completion already happens in a poller, so nothing more
...
static void usb9pfs_add_automount(struct usb_configuration *c)
{
	...
	udc_name = usb_gadget_get_udc_name(cdev->gadget);

	path = xasprintf("/mnt/9p/%s", udc_name);
	cmd = xasprintf("mount -t 9p -o trans=usbg,msize=16384,aname="
			"/home/${global.user}/nfsroot/${global.hostname}"
			" %s %s",
			udc_name, path);

	if (mkdir(path, 0755) == 0) {
		ret = automount_add(path, cmd);
		if (ret)
...
device_initcall(usb9pfs_modinit);
